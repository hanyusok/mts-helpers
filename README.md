...
| 경로 (URL) | 서비스명 | 설명 |
| :--- | :--- | :--- |
| `http://localhost:3001/` | **김기중 소아청소년과의원 메인** | 진료시간, 의료진(김기중 원장), 병원 위치(매교역 2번 출구), 실시간 진료 상태, 주요 서비스 바로가기 |
| `http://localhost:3001/admin/` | **병원 정보 관리 콘솔** | 병원명, 진료시간, 주소, 네이버 지도 링크, 원장 소개 및 진료과목을 웹에서 실시간 편집 및 저장 |
| `http://localhost:3001/quick/` | **모바일 간편 접수** | 스마트폰으로 성명 + 생년월일 입력하여 당일 진료 원터치 접수 |
| `http://localhost:3001/kiosk/` | **원내 무인 키오스크** | 원내 태블릿/터치 키오스크 기기에서 이름과 생년월일을 입력하여 간편 접수 |
| `http://localhost:3001/quicklist/` | **실시간 대기열** | 환자가 자신의 진료 순서와 실시간 대기 인원을 스마트폰으로 확인 |
| `http://localhost:3001/signage/` | **대기실 DID 전광판** | 대기실 대형 모니터/TV용 현재 진료 중 및 다음 대기자 안내 화면 |
...
├── kiosk-ui/               # 원내 무인 키오스크 웹 UI (/kiosk/)
...
├── config.py               # 서버 포트(3001), dev/prod 환경 및 병원 설정 관리
...
scripts\run_dev.bat
```
브라우저에서 `http://localhost:3001` 으로 접속하여 정상 작동을 확인합니다.
...
1. **웹 브라우저 UI를 통한 실시간 변경 (가장 편리한 방법)**:
   - 메인 화면(`http://localhost:3001/`) 우측 상단의 `[병원 정보 관리 ⚙️]` 버튼을 누르거나,
   - `http://localhost:3001/admin/` 으로 접속하여 병원명, 진료시간, 위치, 네이버 지도 링크, 의료진 소개를 손쉽게 수정하고 [저장하기]를 누르면 즉시 전체 시스템에 반영됩니다.
2. **설정 파일 직접 수정**:
...
* **김기중의원 공인인증서 설치 경로**:
  ...
  - 웹 관리 콘솔(`http://localhost:3001/admin/`)의 **'5. 김기중소아청소년과의원 공인인증서 설치 및 연동 안내'**에서도 상세 가이드를 언제든지 확인할 수 있습니다.
