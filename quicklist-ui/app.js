...
let myStoredRoomCode = localStorage.getItem('quick_patient_room_code') || "";
let myStoredRoomName = localStorage.getItem('quick_patient_room_name') || "";

let currentRoomFilter = 'all';
let activeDoctors = [];
let latestQueue = [];
let latestRoomsSummary = {};
...
function clearMyPatient() {
    ...
    localStorage.removeItem('quick_patient_room_code');
    localStorage.removeItem('quick_patient_room_name');
    myStoredName = "";
    myStoredBirth = "";
    myStoredResid1 = "";
    myStoredRoomCode = "";
...
async function loadWaitlistData(silent = false) {
    ...
    try {
        ...
        activeDoctors = data.doctors || [];
        latestRoomsSummary = data.rooms_summary || {};

        renderRoomTabs(activeDoctors, latestQueue);
        renderFilteredQueue();
...
function renderRoomTabs(doctors, queue) {
    const tabsBar = document.getElementById("room-tabs-bar");
    if (!tabsBar) return;
    ...
    let html = `
        <button type="button" class="room-tab ${currentRoomFilter === 'all' ? 'active' : ''}" onclick="switchRoomTab('all')">
            <i class="fa-solid fa-list-ul"></i> 전체 진료실
            <span class="tab-count">${totalCount}</span>
        </button>
    `;

    (doctors || []).forEach(d => {
        const rcStr = String(d.room_code || 1);
        const count = activeQueue.filter(p => String(p.room_code || p.doc || '1') === rcStr).length;
        const isActive = currentRoomFilter === rcStr;
        html += `
            <button type="button" class="room-tab ${isActive ? 'active' : ''}" onclick="switchRoomTab('${rcStr}')">
                <i class="fa-solid fa-door-open"></i> ${d.room_name || `제${rcStr}진료실`} ${d.doctor_name || ''}
...
function switchRoomTab(roomKey) {
    currentRoomFilter = String(roomKey);
    renderRoomTabs(activeDoctors, latestQueue);
    renderFilteredQueue();
}
...
function renderFilteredQueue() {
    ...
    const activeQueue = (latestQueue || []).filter(p => !p.fin || p.fin.trim() !== '*');

    // Filter by selected room tab
    let displayedQueue = activeQueue;
    if (currentRoomFilter !== 'all') {
        displayedQueue = activeQueue.filter(p => String(p.room_code || p.doc || '1') === currentRoomFilter);
    }

    // Update Counter Badges
    const isTreating = displayedQueue.length > 0;
    countTreating.textContent = isTreating ? "1명" : "0명";
    countWaiting.textContent = `${displayedQueue.length}명`;
    listTotalBadge.textContent = `대기 ${Math.max(0, displayedQueue.length - 2)}명`;

    // Spot 1: Currently Treating (displayedQueue[0])
    if (displayedQueue.length > 0) {
        const p0 = displayedQueue[0];
        ...
        treatingSub.textContent = p0.doctor ? `${p0.doctor} ${p0.doctor_title || '원장'} 진료 진행 중` : "진료실에서 진료 진행 중";
    } else {
        treatingName.textContent = "진료 준비 중";
        treatingName.classList.add('no-data');
        treatingRoom.textContent = "-";
        treatingSub.textContent = currentRoomFilter !== 'all' ? "해당 진료실 대기 환자가 없습니다" : "현재 진료 대기 중인 환자가 없습니다";
    }

    // Spot 2: Next Patient (displayedQueue[1])
    if (displayedQueue.length > 1) {
        const p1 = displayedQueue[1];
    ...
    if (listContainer) {
        ...
        if (displayedQueue.length > 2) {
            const restQueue = displayedQueue.slice(2);
            restQueue.forEach((p, idx) => {
                ...
                const rCode = p.room_code || p.doc || 1;
                const roomClass = `room-${(rCode % 4) || 1}`;
                ...
                li.innerHTML = `
                    ...
                    <div class="item-right">
                        <span class="item-room ${roomClass}">${p.room || '1번 진료실'}</span>
...
function updateMyWaitStatus(activeQueue) {
    ...
    if (myIdx === -1) {
        ...
        // Currently treating
        const patient = activeQueue[0];
        const roomName = patient.room || myStoredRoomName || "진료실";
        banner.style.display = 'block';
        posEl.textContent = "현재 진료 중!";
        posEl.style.fontSize = "1.2rem";
        posEl.className = "my-stat-value primary";
        aheadEl.textContent = "0명 (입장)";
        hintEl.innerHTML = `<strong>${myStoredName}</strong>님, <strong>${roomName}</strong>로 입장해 주세요!`;
    } else if (myIdx === 1) {
        // Next in line
        const patient = activeQueue[1];
        const roomName = patient.room || myStoredRoomName || "진료실";
        banner.style.display = 'block';
        ...
        hintEl.innerHTML = `다음 차례입니다. <strong>${roomName}</strong> 문 앞 대기실에서 잠시만 기다려 주세요.`;
    } else {
        // Position 3 or later
        const patient = activeQueue[myIdx];
        const roomName = patient.room || myStoredRoomName || "진료실";
        banner.style.display = 'block';
        ...
        hintEl.innerHTML = `<strong>${roomName}</strong> 대기 중입니다. 순서가 가까워지면 원내 대기실에서 대기해 주세요.`;
    }
}
