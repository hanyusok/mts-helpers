...
3. **스크립트가 자동으로 수행하는 작업**:
   ...
   * **Windows 고급 방화벽**에 포트 `3001 (TCP)` 인바운드 허용 규칙 자동 등록 (원내 스마트폰/태블릿/모니터 접속용)
...
3. 웹 브라우저를 열고 다음 주소에 접속하여 확인합니다:
   * **메인 안내 페이지**: `http://localhost:3001/`
   * **모바일 간편접수**: `http://localhost:3001/quick/`
   * **원내 무인 키오스크**: `http://localhost:3001/kiosk/`
   * **실시간 대기열**: `http://localhost:3001/quicklist/`
   * **대기실 DID 전광판**: `http://localhost:3001/signage/`
...
* 스마트폰에서 `http://192.168.0.50:3001/quick/` 으로 접속합니다.
* 접속이 차단된다면 `1_install_clean_pc_prerequisites.ps1`을 관리자 권한으로 실행하여 방화벽 3001 포트가 열려 있는지 다시 확인하세요.
